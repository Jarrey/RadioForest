...
echo "Running syncInternetRadio.py with target=$SYNC_TARGET_DIR backup=$SYNC_BACKUP_DIR" >> "$LOG_FILE"
...
python3 /var/www/html/scripts/syncInternetRadio.py "$@" >> "$LOG_FILE" 2>&1
STATUS=$?
