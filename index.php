...
<body>
    <div class="container">
        ...
        <div class="filter-section">
            ...
            <div class="filter-content collapsed" id="typesContent">
                <div class="types" id="typeBtns"></div>
            </div>
        </div>
        
        <div class="player-bar" id="playerBar">
            <div class="player-logo placeholder" id="playerLogo">📻</div>
    ...
    <script>
        ...
        let isLoading       = false;
        let currentUrl      = '';
        let currentStation  = null;  // 当前加载的电台对象
        ...
        $(function () {
            ...
            function renderStations(stations) {
                ...
                stations.forEach(station => {
                    ...
                    const active = station.url === currentUrl ? ' active' : '';
                    ...
                    html += `<div class="station-card${active}" data-url="${station.url}">${logoHtml}<div class="station-content"><div class="station-name">${station.name}</div><div class="station-meta">${regionTagHtml}${station._typeTagHtml}</div></div></div>`;
                });
            ...
            function playStation(station) {
                ...
                if (imgSrc) {
                    ...
                    $('#fullscreenCover').replaceWith(`<div class="fullscreen-cover placeholder" id="fullscreenCover">📻<div class="sound-wave" id="soundWave"><div class="sound-wave-bar"></div><div class="sound-wave-bar"></div><div class="sound-wave-bar"></div><div class="sound-wave-bar"></div><div class="sound-wave-bar"></div></div></div>`);
                }
                    
                // 标记当前卡片
                $('.station-card').removeClass('active')
                    .filter(function () { return $(this).attr('data-url') === station.url; })
                    .addClass('active');
            ...
            function updateURL() {
                ...
                if (currentUrl)               params.set('play',   currentUrl);
            ...
            function init() {
                ...
                if (playUrl) {
                    const station = allStations.find(s => s.url === playUrl);
                    if (station) {
                        // 自动播放 URL 中指定的电台
                        playStation(station);
                    }
            ...
            $('#stationsGrid').on('click', '.station-card', function () {
                ...
                const station = allStations.find(s => s.url === url);
            ...
            $('#audioPlayer')
                ...
                .on('waiting', function () {
                    $('#playerStatus, #fullscreenStatus').text('缓冲中...');
                })
                .on('error', function () {
                    $('#statusDot, #fullscreenDot').removeClass('playing').addClass('paused');
            ...
            $('#fullscreenBtn').on('click', function(e) {
                e.stopPropagation();
                isAutoFullscreen = false; // 标记为手动触发
                showFullscreen();
                // 同时请求浏览器全屏
            ...
            // 关闭全屏
            $('#fullscreenClose').on('click', hideFullscreen);
            ...
            function checkOrientation() {
                ...
                if (enteredLandscape) {
                    isAutoFullscreen = true; // 标记为自动触发
                    showFullscreen();
                    // 延迟请求浏览器全屏，避免阻塞
                    setTimeout(() => requestBrowserFullscreen(), 100);
                } else if (!isLandscape) {
                    // 竖屏时只关闭自动触发的全屏，手动点击的保留
                    if ($('#fullscreenPlayer').hasClass('show') && isAutoFullscreen) {
                        // 竖屏自动退出，直接操作DOM，不设置手动关闭标志
                        $('#fullscreenPlayer').removeClass('show');
                        $('body').css('overflow', '');
                        isAutoFullscreen = false;
            ...
            $(document).on('fullscreenchange webkitfullscreenchange mozfullscreenchange', function() {
                // 如果浏览器退出了全屏，也关闭我们的全屏播放器
                if (!document.fullscreenElement && !document.webkitFullscreenElement && !document.mozFullScreenElement) {
                    if ($('#fullscreenPlayer').hasClass('show')) {
                        isAutoFullscreen = false; // 清除自动全屏标志
                        $('#fullscreenPlayer').removeClass('show');
                        $('body').css('overflow', '');
                    }
                }
